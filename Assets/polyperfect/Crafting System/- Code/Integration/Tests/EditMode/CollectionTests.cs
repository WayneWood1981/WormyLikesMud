...
namespace Polyperfect.Crafting.Integration.Tests
{
    public class CollectionTests
    {
        ...
        public void SlottedInventoryTests()
        {
            ...
            for (var i = 0; i < 4; i++)
                slottedInventory.AddSlot(new ItemStackSlot());
            ...
            Assert.AreEqual(1, slottedInventory.SlotList.Select(s=>s.Peek()).IndexOf(i4));
        ...
        public void InventoryOpsTesting()
        {
            ...
            for (var i= 0; i < 3; i++)
                smallInventory.AddSlot(new ItemStackSlotWithCapacity(3));
            var largeInventory = new SlottedInventory<ItemStackSlotWithCapacity>();
            for (var i= 0; i < 3; i++)
                largeInventory.AddSlot(new ItemStackSlotWithCapacity(999));
