...
namespace Polyperfect.Crafting.Edit
{
    public static class VisualElementPresets
    {
        ...
        {
            var categories = EditorItemWorld.Instance.ActiveFragments.SelectMany(f => f.CategoryObjects).Distinct().ToArray();
            return new MemberGroupControl<BaseCategoryObject>(
                ...
                {
                    v.Clear();
                    v.Add(new Label(catObj.name.SpacifyCamelCaps().WordwrapOrTruncate(20,2)).SetWrap());
                },
                (v, catObj) =>
                {
                    v.Clear();
                    v.Add(catObj.CreateInlineEditor(obj, catObj.name.SpacifyCamelCaps().WordwrapOrTruncate(20,2)).SetWrap());
                },
                f => true);
        }
    }
