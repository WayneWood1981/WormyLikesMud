...
namespace Polyperfect.Crafting.Edit
{
    ...
    public class BaseObjectWithIDDrawer : PropertyDrawer
    {
        public override VisualElement CreatePropertyGUI(SerializedProperty property)
        {
            ...
            var icon = EditorItemWorld.Instance.GetReadOnlyAccessor<IconData>(StaticCategories.Icons).GetDataOrDefault(obj);
            ...
            var displayNameText = obj?obj.name.SpacifyCamelCaps().WordwrapOrTruncate(8,2):"";
            if (!icon.Icon && obj)
            {
                slot.Add(new Label(displayNameText).SetStretch().CenterContents());
            }
            ...
            slot.Add(VisualElementPresets.CreateHoverText(slot, displayNameText));
            if (obj)
