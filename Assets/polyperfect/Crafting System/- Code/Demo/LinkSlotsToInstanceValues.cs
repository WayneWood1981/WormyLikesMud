...
namespace Polyperfect.Crafting.Demo
{
    ...
    public class LinkSlotsToInstanceValues : ItemUserBase
    {
        ...
        void Awake()
        {
            var capacityConstraint = new MaxCapacityConstraint() { };
            AffectedSlot.AddConstraint(capacityConstraint);
            InstanceSlot.Changed += () =>
            {
                capacityConstraint.Capacity = World.GetReadOnlyAccessor<int>(MaximumSource).GetDataOrDefault(InstanceSlot.Peek().ID);
