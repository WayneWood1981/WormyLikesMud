...
namespace Polyperfect.Common
{
    public static class TypeExtensions
    {
        ...
        public static PropertyInfo GetPropertyViaPath([NotNull] this Type type, string path)
        {
            var parent = type;
            var pi = parent.GetProperty(path,
                GetBindingFlags());
            var paths = path.Split('.');
            for (var i = 0; i < paths.Length; i++)
            {
                while (true)
                {
                    pi = parent.GetProperty(paths[i], GetBindingFlags());
                    if (pi == null)
                        if (parent.BaseType != null)
                        {
                            parent = parent.BaseType;
                ...
                {
                    parent = pi.PropertyType.GetElementType();
                    i += 2;
                    continue;
                }
                ...
                {
                    parent = pi.PropertyType.GetGenericArguments()[0];
                    i += 2;
                    continue;
                }

                parent = pi.PropertyType;
            }
        ...
        public static object GetOwningObject(this SerializedProperty that)
        {
            var path = that.propertyPath;
            var parent = that.GetTypeOfPropertySO();
            var fi = parent.GetField(path,
                GetBindingFlags());
            var paths = path.Split('.');
            object ret = that.serializedObject.targetObject;
